...
from Interpolation_of_coeffs import *  # for cl and cd
...
def unsteady_BEM_function(time_interval = 100, dt = 0.15, vs = [11.4], Tower_influ=0, Wind_shear=0, Dynamic_wake=0, Dynamic_stall=0, Turbulence=0, Controller = 0, Control_Vect = []):
    #areolastic dynamics
    #------------------------
    ...
    for i in range(int(N)): #time loop
        ...
        for j in range(B): #Loop each blade
            ...
            for k in range(len(b_elm)-1): #Loop each element on blade
                ...
                Vrel_z =  Vz + Wz[i,j,k] 
                Vrels = Vrel_y**2 + Vrel_z**2 #relativE wind

                flow_angle = np.arctan(Vrel_z/(-Vrel_y)) #find flow angle
                aot = flow_angle - (twist[k]*np.pi/180 + theta_pitch[i]) #angle of attack
                
                [Cl_inst,Cd,cmom,fs0,cl_inv,clfs] = force_coeffs_10MW(aot*180/np.pi,t_c[k],aoa_tab,cl_tab,cd_tab,cm_tab,f_stat,cl_inv_tab,cl_fs) #interpolate cl cd
                ...
                l = 0.5* rho * Vrels * c[k] * Cl[i+1,j,k] #lift (Vrels = Vrel^2)
                d = 0.5* rho * Vrels * c[k] * Cd # drag
                
                
                pn[i+1,j,k] = l * np.cos(flow_angle) + d*np.sin(flow_angle) # normal loads [N/m]
                pt[i+1,j,k] = l * np.sin(flow_angle) - d*np.cos(flow_angle) # tangential loads [N/m]
        ...
        if Controller: #apply controller
            ...
            theta_set_point = theta_P[i+1] + theta_I[i+1] #theta set point
